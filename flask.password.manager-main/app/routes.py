...
def login():
    if request.method == 'POST':
        ...
        # Kullanıcı adı ve şifreyi statik olarak kontrol et
        if username == STATIC_USERNAME and password == STATIC_PASSWORD:
            flash('Login successful!', 'success')
            session['user_id'] = username  # Oturumu başlat
...
# This code block is handling the addition of a new password to the database.
    if request.method == 'POST':
        ...
        new_password = Password(service_name=service_name, username=username, encrypted_password=encrypted_password)
...
@app.route('/edit_password_service_name/<int:id>', methods=['GET', 'POST'])
def edit_password_service_name(id):
    ...
    if password:
        if request.method == 'POST':
            ...
            password.service_name = service_name
            db.session.commit()
            flash('Service name edited successfully!', 'success')
            return redirect(url_for('index'))
        return render_template('edit_password_service_name.html', password=password)
    else:
        flash('Password not found', 'danger')
        return redirect(url_for('index'))
